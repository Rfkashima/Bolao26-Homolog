...
function scheduleInitialBackendLoad() {
  window.requestAnimationFrame(() => {
    window.setTimeout(() => {
      ...
      const requests = [loadBaseStateWithRetry()];
...
function loadBaseStateWithRetry(attempt = 0) {
  return loadBaseState().catch((error) => {
    if (attempt >= 2) {
      throw error;
    }
    ...
    }).then(() => loadBaseStateWithRetry(attempt + 1));
  });
}
...
function getHomePicksMatch() {
  ...
  let match = state.homePicksMatchId
    ? matches.find((item) => item.id === state.homePicksMatchId) || null
    : null;

  if (!state.loadedBackend && !state.homePicksManual) {
    return { match: null, matches, index: -1 };
  }

  if (!state.homePicksManual) {
    match = getLastFinishedMatch() || match || matches[0];
  }

  if (!match) {
    match = getLastFinishedMatch() || matches[0];
